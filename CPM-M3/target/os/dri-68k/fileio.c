 ...
 * revisions:
 *
 *	2005-04-09 rli: fixed byte-order assumptions in setran, fsize,
 *	  and getsize.
 *
 *	2010-08-07 rli: worked around some compiler complaints about
 *	  casts on a couple of lvalues in close.
 *
 *	2013-04-25 rli: although alloc is declared as a boolean function
 *	  and dirscan relies on its return value, the function does not
 *	  return anything. seldsk passes a parameter that makes dirscan
 *	  ignore the return value, but it must return something; make it
 *	  return 0.
 *
 *	2013-06-07 rli: include config.h so the host tools can #define away
 *	  the gcc packing attributes.
 *
 ***/
    ...
    {                   /* Note that FCB merging is done here as a final
        ...
        {               /* Large disk map merge routine */
            ...
            do
            {
                ...
                (UWORD *)dp += 1;
#endif
                fp += sizeof( UWORD ); /* gcc no longer likes the above */
                dp += sizeof( UWORD );
                i -= 1;
            } while (i);
        }

 ...
 * revisions:
 *
 *	2010-08-07 rli: terminate command table with a NULL command name
 *	  instead of a fixed number of commands, allowing new commands
 *	  to be added more easily. Sprinkling of typecasts. Change name
 *	  of entry point from main() to ccp().
 *
 *	2013-06-05 rli: include config.h and add callouts for decoding
 *	  and execution of unrecognized commands.
 *
 *	2014-01-04 rli: add loader error messages for return values 4
 *	  and 5.
 *
 ***/
...
BYTE  lderr4[] = "Wrong architecture$";
BYTE  lderr5[] = "Wrong region$";
...
#ifdef config_hasccphooks_c

	/* check for platform-specific extended command.
	 */

	i = platform_ccpdecode( cmd );
	if( i != 0 ) return i;
	...
	if(open)
	{
	   ...
	   switch( (*ldrpgm) (glb_index) )
	   {
		...
		case 3: bdos(PRINT_STRING,lderr3); break;
                case 4: bdos(PRINT_STRING,lderr4); break;
                case 5: bdos(PRINT_STRING,lderr5); break;
...
#ifdef config_hasccphooks_c
				if( platform_ccpexecute( cmdno ) ) break;
#endif
				echo_cmd(parm,BAD);
	}
}
